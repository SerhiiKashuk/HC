...
import stat.RedirectRequestRecord;
import stat.ServerConnectionRecord;
import stat.ServerRequestRecord;
...
public class ServerHandler extends ChannelHandlerAdapter {
    ...
    private static DefaultChannelGroup allChannels = 
    		new DefaultChannelGroup("netty-receiver", ImmediateEventExecutor.INSTANCE);
	...
	sb.append(
		...
		.append(DBConnection.NUMBER_OF_REQUESTS_OUTPUT).append(")</caption>")
	...
	ListIterator<ServerConnectionRecord> iterator = HChannelTrafficShapingHandler
	...
	response.getHeaders().set(CONTENT_TYPE, "text/html");
	response.getHeaders().set(CONTENT_LENGTH,
		response.content().readableBytes());
	if (!keepAlive) {
	    ctx.write(response).addListener(ChannelFutureListener.CLOSE);
	} else {
	    response.getHeaders().set(CONNECTION,
